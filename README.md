...
Было принято решение разделить хранение данных на несколько БД по следующему принципу:
- База данных постов (database_post_system).
- База данных медиа файлов (database_media_system)
- База данных комментариев к постам (database_comment_system)
- База данных подписок (database_subscription_system)
- База данных пользователей (database_user_system)


### 3.1. База данных постов (database_post_system)
Шардирование по post_id (key based). Все посты, лайки, популярные места распределяются равномерно.

Таблица с постами партиционируется по месяцам. Это позволит все свежие посты быстрей. Основная нагрузка на посты наверняка будет за последние периоды. При поиске глубже также можно ограничивать запрос датами и смотреть и точно попадать в партиции.

Хосты: Один хост на 16 терабайт на мастер.

Два хоста на две реплики по 16 терабайт.

### 3.2. База данных медиа файлов (database_media_system)

Равномерное и рэндомное распределение на несколько шардов. Партиционирование не предусматривается, ввиду отсутствия необходимости. 

Хосты: На каждый хост будем брать по 5 дисков. Всего дисков 122. Итого 25 хостов на мастер. 50 хостов будет на 2 реплики. 


### 3.3. База данных комментариев к постам (database_comment_system)
Шардирование по post_id (key based). 
Партиционирование не предусматривается.

Хосты: Один хост на 16 терабайт на мастер.

Два хоста на две реплики по 16 терабайт.

### 3.4. База данных подписок (database_subscription_system)
Шардирование по user_id (key based).
Партиционирование по user_id (key based).

Хосты: Один хост на 1 терабайт на мастер. 

Два хоста на две реплики по 1 терабайту.

### 3.4. База данных пользователей (database_user_system)
Шардирование по user_id (key based).

Хосты:
Не считал ранее. Оставил для консистентности проекта. 

## 4. Расчет оценки хостов
